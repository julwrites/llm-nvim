...
describe('llm.commands', function()
  ...
  describe('interactive_prompt_with_fragments', function()
    it('should guide the user to select fragments and a prompt', function()
      local select_call_count = 0
      local select_spy = spy.new(function(items, opts, on_confirm)
        select_call_count = select_call_count + 1
        if select_call_count == 1 then
          on_confirm(items[1]) -- Select existing fragment
        elseif select_call_count == 2 then
          on_confirm(items[1]) -- Select the first fragment
        else
          on_confirm(items[5]) -- Done
        end
      end)
      local input_spy = spy.new(function(opts, on_confirm)
        on_confirm('test interactive prompt')
      end)
      _G.vim.ui = {
        select = select_spy,
        input = input_spy,
      ...
      local fragments_manager_mock = {
        get_fragments = spy.new(function()
          return { { hash = 'hash1', aliases = { 'alias1' }, content = 'test content' } }
        end),
      }
      ...
      -- We need to manually call the first select
      _G.vim.ui.select({ '1', '2', '3', '4', '5' }, { prompt = 'Add fragments' }, function(choice)
        if choice == '1' then
          _G.vim.ui.select({ 'alias1' }, { prompt = 'Select fragment' }, function()
            _G.vim.ui.select({ '1', '2', '3', '4', '5' }, { prompt = 'Add fragments' }, function(c)
              if c == '5' then
                _G.vim.ui.input({ prompt = 'Enter prompt' }, function(p)
                  commands.prompt(p, { 'alias1' })
                end)
              end
            end)
          end)
