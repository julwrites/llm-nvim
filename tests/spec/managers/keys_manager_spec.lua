...
describe('llm.managers.keys_manager', function()
  ...
  describe('populate_keys_buffer', function()
    it('should set lines in the buffer', function()
      stub(KeysManager, 'get_stored_keys', function()
        return { { name = 'openai', is_set = true }, { name = 'my_custom_key', is_set = true } }
      end)
      local styles = require('llm.ui.styles')
      stub(styles, 'setup_highlights')
      stub(styles, 'setup_buffer_syntax')
      local nvim_buf_set_lines_spy = spy.on(vim.api, 'nvim_buf_set_lines')
      vim.b = { [1] = {} }
      KeysManager.populate_keys_buffer(1)
  ...
  describe('get_provider_info_under_cursor', function()
    it('should return provider info', function()
      stub(vim.api, 'nvim_win_get_cursor', function() return {1, 0} end)
      vim.b = { [1] = { line_to_provider = { [1] = 'openai' }, key_data = { openai = { is_set = true } } } }
      local provider_name, key_info = KeysManager.get_provider_info_under_cursor(1)
      assert.are.equal('openai', provider_name)
      assert.is_true(key_info.is_set)
      mock.revert(vim.api)
    end)
  end)

  describe('manage_keys', function()
    it('should call open_specific_manager', function()
      local unified_manager = require('llm.ui.unified_manager')
      stub(unified_manager, 'open_specific_manager')
      KeysManager.manage_keys()
      assert.spy(unified_manager.open_specific_manager).was_called_with('Keys')
      mock.revert(unified_manager)
    end)
  end)
end)
