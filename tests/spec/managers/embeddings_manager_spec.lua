...
describe("embeddings_manager", function()
  ...
  it("should run embed_models command", function()
    embeddings_manager.embed_models({ subcommand = "default" })
    assert.spy(llm_cli.run_llm_command).was_called_with("embed-models default")
  end)
end)
