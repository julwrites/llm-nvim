...
function M.debug_custom_openai_models()
  local config_dir, yaml_path = file_utils.get_config_path("extra-openai-models.yaml")
...
function M.create_sample_yaml_file()
  local config_dir, yaml_path = file_utils.get_config_path("extra-openai-models.yaml.sample")
...
function M.delete_custom_openai_model(model_id)
  ...
  for _, model in ipairs(models_list) do
    if model.model_id ~= model_id then
