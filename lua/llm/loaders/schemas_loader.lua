      ...
      if success and parsed then
        ...
        for name, id_val in pairs(parsed) do
          ...
          if type(id_val) ~= "string" or not id_val:match("^[0-9a-f]+$") then
             if config.get("debug") then
                vim.notify("Skipping invalid schema entry in schemas.json: name=" .. name .. ", value=" .. vim.inspect(id_val), vim.log.levels.WARN)
             end
          ...
          if schemas[schema_id_str] then
            ...
            if config.get("debug") then
              ...
              vim.notify("Found schema in config but not in llm CLI: " .. schema_id_str .. " (" .. name .. ")", vim.log.levels.DEBUG)
            end
          end
...
function M.get_schema(schema_id_or_name)
  ...
  for line in result:gmatch("[^\r\n]+") do
    ...
    if found_target_id and looking_for_schema_start then
      ...
      elseif line:match(id_pattern) then
        ...
        if config.get("debug") then vim.notify("Found next ID before schema start for " .. schema_id, vim.log.levels.WARN) end
    ...
    for line in result:gmatch("[^\r\n]+") do
      ...
      elseif not in_target_schema_section then
        if line:match(".*schema:.*") then
          in_target_schema_section = true
          if config.get("debug") then vim.notify("Alt approach: Found schema marker in line: " .. line, vim.log.levels.DEBUG) end
        end
      elseif in_target_schema_section then
        if line:match("^%s+") then
          local schema_line = line:gsub("^%s+", "")
  ...
  if schema_text ~= "" then
    ...
    if config.get("debug") then
      ...
      if result:find(schema_id) then
        vim.notify("Schema ID " .. schema_id .. " was found in the output but schema content couldn't be parsed", vim.log.levels.DEBUG)
    ...
    -- As a last resort, try to extract any JSON-like content from the output
    if result:match("{") and result:match("}") then
      local potential_json = result:match("({.-})")
      if potential_json then
        local success, _ = pcall(vim.fn.json_decode, potential_json)
        if success then
          if config.get("debug") then vim.notify("Found potential JSON in output, using it as schema", vim.log.levels.DEBUG) end
          schema_content = potential_json
        end
      end
    end
  ...
  -- Save to local file if we have a name
    if schema_name then
    ...
    -- The 'end' above closes the 'if schema_name then' block for saving locally
    
    -- Return the successfully parsed schema details from CLI (moved after the save block)
    return {
      id = schema_id,
...
-- Removed extra 'end' here
end -- This 'end' closes the function M.get_schema
...
function M.run_schema(schema_id_or_name, input, is_multi)
  ...
  if config.get("debug") then
    vim.notify("Schema input content:\n" .. input, vim.log.levels.DEBUG)
  end
  ...
  if schema_id and type(schema_id) == "string" and schema_id:match("^[0-9a-f]+$") then
    ...
    -- Use the temporary schema file path
    cmd = string.format("cat '%s' | llm %s '%s'", temp_file, schema_flag, schema_temp_file)
...
function M.run_schema_with_url(schema_id_or_name, url, is_multi)
  ...
  if not success then
    ...
    vim.notify("Failed to run schema with URL content. Check your schema ID.", vim.log.levels.ERROR)
    return nil
  end
  ...
  local function validate_properties(props, path)
    ...
    for key, value in pairs(props) do
      ...
      if not key_valid then
        if require('llm.config').get("debug") then
           vim.notify(string.format("Manual Validation FAILED for key: %s (stripped: %s, len: %d)", vim.inspect(key), vim.inspect(stripped_key), key_len), vim.log.levels.DEBUG)
        end
        -- Use the original key in the error message for clarity
        return false, string.format("Invalid property key '%s' at path '%s'. Keys must match ^[a-zA-Z0-9_-]{1,64}$.", key, path)
      end
