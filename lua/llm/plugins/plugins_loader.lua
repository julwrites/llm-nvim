-- llm/plugins/plugins_loader.lua - Plugin data loader for llm-nvim
-- License: Apache 2.0
...
local function get_hardcoded_plugins()
  ...
  plugins["llm-mlx"] = { name = "llm-mlx", description = "Run models using Apple MLX framework", category =
  "Local models" }
  plugins["llm-ollama"] = { name = "llm-ollama", description = "Use Ollama models via API", category = "Local models" }
  plugins["llm-llamafile"] = { name = "llm-llamafile", description = "Run models via llamafile", category =
  "Local models" }
  ...
  plugins["llm-anyscale-endpoints"] = { name = "llm-anyscale-endpoints", description = "Use Anyscale Endpoints API", category =
  "Remote APIs" }
  ...
  plugins["llm-bedrock-anthropic"] = { name = "llm-bedrock-anthropic", description = "Use AWS Bedrock Anthropic models", category =
  "Remote APIs" }
  plugins["llm-bedrock-meta"] = { name = "llm-bedrock-meta", description = "Use AWS Bedrock Meta models", category =
  "Remote APIs" }
  ...
  -- Embedding models
  plugins["llm-sentence-transformers"] = { name = "llm-sentence-transformers", description =
  "Generate embeddings with sentence-transformers", category = "Embedding models" }
  plugins["llm-clip"] = { name = "llm-clip", description = "Generate embeddings with CLIP", category = "Embedding models" }
  plugins["llm-embed-jina"] = { name = "llm-embed-jina", description = "Generate embeddings with Jina", category =
  "Embedding models" }
  plugins["llm-embed-onnx"] = { name = "llm-embed-onnx", description = "Generate embeddings with ONNX", category =
  "Embedding models" }

  -- Extra commands
  plugins["llm-cmd"] = { name = "llm-cmd", description = "Run shell commands with LLM", category = "Extra commands" }
  plugins["llm-cmd-comp"] = { name = "llm-cmd-comp", description = "Command completion with LLM", category =
  "Extra commands" }
  ...
  plugins["llm-templates-github"] = { name = "llm-templates-github", description = "Load templates from GitHub", category =
  "Fragments and template loaders" }
  plugins["llm-templates-fabric"] = { name = "llm-templates-fabric", description = "Load templates from Fabric", category =
  "Fragments and template loaders" }
  plugins["llm-fragments-github"] = { name = "llm-fragments-github", description = "Load fragments from GitHub", category =
  "Fragments and template loaders" }
  plugins["llm-hacker-news"] = { name = "llm-hacker-news", description = "Load content from Hacker News", category =
  "Fragments and template loaders" }

  -- Just for fun
  plugins["llm-markov"] = { name = "llm-markov", description = "Generate text with Markov chains", category =
  "Just for fun" }
...
function M.refresh_plugins_cache()
  ...
  if plugins and vim.tbl_count(plugins) > 0 then
    ...
    plugins_cache = plugins
    vim.notify("Successfully refreshed plugin directory with " .. vim.tbl_count(plugins) .. " plugins",
      vim.log.levels.INFO)
  else
...
function M.parse_debug_html()
  ...
  vim.notify(string.format("Alternative parser: found %d plugins", vim.tbl_count(plugins_alternative)),
    vim.log.levels.INFO)
