...
2. [Gap 3] - [Low Impact/Low Effort] - Missing support for explicitly disabling logging via `--no-log` (`-n, --no-log`).
3. [Gap 4] - [Low Impact/Low Effort] - Missing support for explicitly enabling logging via `--log` (`--log`).
4. [Gap 5] - [Low Impact/Medium Effort] - Missing commands for logs management in editor context (`llm logs list`).
5. [Gap 1] - [Medium Impact/Medium Effort] - Missing support for managing available embedding models (`llm embed-models`).
