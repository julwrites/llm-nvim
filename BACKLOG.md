...
## Ranked Backlog
1. [Bug 6] - [High Impact/Low Effort] - Remove unnecessary manual `os.remove()` calls for files generated by `vim.fn.tempname()`.
2. [Gap 11] - [High Impact/Medium Effort] - Add support for Model Aliases (`llm aliases`).
3. [Gap 12] - [High Impact/Medium Effort] - Implement SSE streaming support in `scripts/llm.py` instead of waiting for full response.
4. [Tech Debt 2] - [High Impact/Medium Effort] - Enhance async job handling logic in `job.lua` for edge cases.
